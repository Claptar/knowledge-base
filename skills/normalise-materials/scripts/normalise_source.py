...
Usage:
    uv run --group convert python skills/normalise-materials/scripts/normalise_source.py \
        sources/berkeley-stat210a/fall-2026 [more sources ...] [--apply]
    ... --all            every source in the lockfile
    ... --include-all    do not skip course administrivia
    ... --summary-only   regenerate docs/reference/SUMMARY.md and index.md, convert nothing
...
Four rules it exists to enforce, each of which cost someone time to learn:
  ...
  * **Never guess in the publishing direction.** `material:` in the lockfile decides the
    destination, per AGENTS.md "What may be republished". Anything unclassified, any book, and
    any source that cannot cite an original goes to the gitignored `reference-private/`.
...
def upstream(entry, relpath, raw=False):
    """URL of one file in the original, or None when the source has no recorded upstream."""
...
def convert_transcript(path):
    sys.path.insert(0, str(REPO / "skills" / "adapt-recordings" / "scripts"))
...
def rewrite_links(text, doc, out_page_dir, entry, index_by_rel, root):
    ...
    def sub(m):
        ...
        url = upstream(entry, rel, raw=bool(bang))
        if url:
...
def process(entry, sources_dir, include_all, apply):
    ...
    out_root, published, reason = destination(entry)
...
def write_reference_index(reference_dir, slugs, apply):
    ...
    body = [
        "---", "title: Reference", "---", "",
        "# Converted reference material", "",
        "Course material converted to markdown and split by section, so that a topic file can",
        "link to a lecture's third argument rather than to a page range in a PDF.",
        "",
        "> **Converted, not adapted.** The text is the author's, reformatted. Rewriting it",
        "> motivation-first is a different job and lives in `adapted/`; the record of working",
        "> through it lives in `topics/`. Every page here cites its source and links to the",
        "> original, and these files are generated and **never edited by hand**.",
        "",
        f"{len(entries)} source{'s' if len(entries) != 1 else ''} converted so far — the full "
        "catalogue, converted or not, is in [Resources](../resources/index.md).",
        "",
        "## Sources", "",
    ]
...
def main():
    ...
    ap.add_argument("--sources", type=Path, default=REPO / "sources")
    ...
    if a.summary_only:
        write_reference_index(reference, set(by_slug), a.apply)
        n = write_nav(reference, a.apply)
    ...
    results = [process(e, a.sources, a.include_all, a.apply) for e in entries]
    ...
    if a.apply:
        write_reference_index(reference, set(by_slug), True)
        n = write_nav(reference, True)
