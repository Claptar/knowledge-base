...
**This skill does not judge sources.** Whether a text is worth his time is `study-mentor` step 4,
which reads `../study-mentor/references/taste.md` and writes the verdict. Hand off rather than
...
This repo publishes a site, so a licence error is the one mistake here with consequences outside
the repo. The full rule is in `../study-mentor/references/kb-structure.md`; what this skill adds is
the failure mode specific to harvesting:
...
the course's licence would route an adaptation to `docs/adapted/` and publish it.
...
- **Catalogue entry per source**, in the right `docs/resources/` subject file, using the format in
  `../study-mentor/references/kb-structure.md`. `Status: unvetted` always — this skill has not read
  it. The slug is the source's identity everywhere.
